...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('en'), 'has-success': fields.en && fields.en.valid }">
    ...
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text"  v-validate="'required'" @input="validate($event)" value="@isset($taskCategory){{ $taskCategory->name }} @endisset" class="form-control" :class="{'form-control-danger': errors.has('en'), 'form-control-success': fields.en && fields.en.valid}" id="en" name="en" placeholder="{{ trans('admin.task-section.columns.name') }}">
        <div v-if="errors.has('en')" class="form-control-feedback form-text" v-cloak>@{{ errors.first('en') }}</div>
    </div>
</div>
@isset($taskCategory)
    @php
        $taskCategory->setLocale('ua');
    @endphp
@endisset
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('ua'), 'has-success': fields.ua && fields.ua.valid }">
    ...
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" value="@isset($taskCategory){{ $taskCategory->name }} @endisset" v-validate="'required'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('ua'), 'form-control-success': fields.ua && fields.ua.valid}" id="ua" name="ua" placeholder="{{ trans('admin.task-section.columns.name') }}">
        <div v-if="errors.has('ua')" class="form-control-feedback form-text" v-cloak>@{{ errors.first('ua') }}</div>
    </div>
</div>
@isset($taskCategory)
    @php
        $taskCategory->setLocale('cz');
    @endphp
@endisset
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('cz'), 'has-success': fields.cz && fields.cz.valid }">
    ...
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" value="@isset($taskCategory){{ $taskCategory->name }} @endisset" v-validate="'required'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('cz'), 'form-control-success': fields.cz && fields.cz.valid}" id="cz" name="cz" placeholder="{{ trans('admin.task-section.columns.name') }}">
